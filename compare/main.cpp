...
int main(int argc, char* argv[]) {
	dirlst_t * inloc, *inrem;
	
	printf("local dir : %s\n", argv[1]);
	inloc = to_dirlst(argv[1]);
	// print_dirlst(inloc);

	printf("remote dir : %s\n", argv[2]);
	inrem = to_dirlst(argv[2]);
	// print_dirlst(inrem);

	printf("COMBINING ..........\n");
	dirlst_t * loc, * rem, * sync;
	comb_loc_rem(inloc, inrem, &loc, &rem, &sync);
...
void print_dirlst(dirlst_t * dl) {
	...
	while (mdp != nullptr) {
		...
		printf("%-10s  |  %s  |  %s\n", mdp->m_name.c_str(), mtime_loc, mtime_rem);
		mdp = mdp->m_next;
		++count;
	}
	printf("Length = %u.\n\n", count);
...
dirlst_t * to_dirlst(char * pathname) {
	...
	for(;;) {
		...
		if (strcmp(dp->d_name, ".") == 0 || strcmp(dp->d_name, "..") == 0) 
			continue;
		*tail = new mdirent_t;
		(*tail)->m_name = std::string(dp->d_name);
